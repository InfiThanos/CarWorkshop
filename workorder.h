...
private:
    ...
    uint16_t orderNumber;   // номер заказа
    float price;            // стоимость работ/заказа
public:
    WorkOrder(uint16_t p_orderNumber, string p_name,            // конструктор
              string p_carBrand, string p_carModel,
              string p_orderInfo, float p_price,
              string p_status);

    void getInfoWorkOrder(uint16_t& p_orderNumber,              // метод для возврата
